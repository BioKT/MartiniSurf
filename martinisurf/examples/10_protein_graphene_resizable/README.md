...
Builds the same protein used in example `09_protein_cnt_3d`, but immobilized on a graphene sheet instead of a nanotube.
...
Workflow:
1. Downloads or reads the same protein used in example 9 (`2A3D`).
...
- This example keeps the same anchor selection as example 9: `--anchor A 1`.
