...
The initial input structure may be provided as a local coordinate file in
PDB or mmCIF/PDBx format, a four-character RCSB Protein Data Bank (PDB)
identifier, or a UniProt accession code.
...
Local mmCIF/PDBx files are converted internally to PDB before the standard
cleaning and martinization steps.
...
\begin{itemize}
  ...
  Multilayer local surfaces use HCP-like ABAB stacking by default and
  can be generated with FCC-like ABCABC stacking using
  \texttt{--surface-stacking fcc}.
...
\texttt{--surface-layers}, \texttt{--surface-stacking}, and
\texttt{--surface-dist-z}. Graphite slabs use \texttt{--graphite-layers}
and \texttt{--graphite-spacing}.
If \texttt{--surface-dist-z} is not provided, local HCP/FCC multilayer
surfaces use the close-packed spacing \(\sqrt{2/3}\,a\), where
\(a\) is the in-plane nearest-neighbor spacing set by \texttt{--dx}.
For local lattice surfaces, MartiniSurf also connects lattice bonds across
periodic X/Y boundaries by default.
...
\begin{itemize}
  ...
  \item \texttt{provenance.json}, which records the command, workflow mode,
  relevant input paths, selected model parameters, and detectable dependency
  versions,
...
\begin{itemize}
  ...
  Handles structure retrieval (PDB/UniProt), local PDB/mmCIF input, cleaning, and
...
\item[\texttt{--pdb}]
Input structure (local \texttt{.pdb}, \texttt{.cif}, or \texttt{.mmcif} file,
RCSB ID, or UniProt ID).
...
\item[\texttt{--el}, \texttt{--eu}, \texttt{--ermd}, \texttt{--ea}, \texttt{--ep}, \texttt{--em}, \texttt{--eb}, \texttt{--eunit}]
Advanced elastic-network parameters passed to \texttt{martinize2}, including
distance cutoffs, minimum residue separation, decay terms, minimum retained
force constant, bead selection, and elastic-network structural unit.
...
\item[\texttt{--go-res-dist}, \texttt{--go-write-file}, \texttt{--go-backbone}, \texttt{--go-atomname}]
Advanced G\=o model controls passed to \texttt{martinize2}, including graph
distance filtering, contact-map output, backbone bead selection, and virtual-site
atom naming.

\item[\texttt{--ss}, \texttt{--collagen}, \texttt{--ed}]
Secondary-structure and extended-region controls passed to \texttt{martinize2}.

\item[\texttt{--martinize-extra-args}]
Controlled passthrough for additional protein-mode \texttt{martinize2} options.
MartiniSurf-managed input, output, topology, force-field, position-restraint,
elastic-network, G\=o, DSSP, and warning-control flags are blocked to preserve
the reproducible output layout.
...
\item[\texttt{--dx}]
In-plane nearest-neighbor lattice spacing for local 2-1 and 4-1
surfaces. The generated lattice uses this value directly.

\item[\texttt{--surface-layers}]
Number of layers for local 2-1 / 4-1 surfaces.

\item[\texttt{--surface-stacking}]
Layer stacking for local 2-1 / 4-1 surfaces:
\texttt{hcp} for ABAB stacking (default) or \texttt{fcc} for ABCABC stacking.

\item[\texttt{--surface-dist-z}]
Manual interlayer spacing for local multilayer surfaces. If omitted,
the default is the close-packed HCP/FCC spacing
\(\sqrt{2/3}\times\texttt{--dx}\).

\item[Periodic local surface connectivity]
Local 2-1 / 4-1 surface lattices include periodic X/Y edge
connectivity by default, so the generated surface remains continuous
...
\item[\texttt{--water-mix}]
Martini~3 protein-workflow option for setting the final \texttt{W}/\texttt{SW}/\texttt{TW}
water composition after solvation. For example,
\texttt{--water-mix SW:0.10,TW:0.10} converts 10\% of waters to
\texttt{SW}, 10\% to \texttt{TW}, and keeps the remaining waters as \texttt{W}.

\item[\texttt{--water-mix-seed}]
Seed parameter for deterministic W/SW/TW water selection.
...
\texttt{--martini-version}, \texttt{--layers}, \texttt{--stacking}, \texttt{--dist-z},
graphite controls, and the full set of CNT flags.
